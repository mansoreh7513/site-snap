...
namespace Snapp.Site.Controllers
{
    ...
    public class PanelController : Controller
    {
        ...
        {
            var result = await _panel.GetUserDetails(User.Identity.Name);
            ...
            ViewBag.MyDate = DateTimeGenerator.GetShamsiDate();
        ...
        public async Task<IActionResult> PaymentCallBack(Guid id)
        {
            ...
            {
                _panel.UpdatePayment(id, DateTimeGenerator.GetShamsiDate(), DateTimeGenerator.GetShamsiTime(), "افزایش اعتبار زرین پال", "زرین پال", result.RefId.ToString(), result.RefId.ToString());
            }
        ...
        public IActionResult CallBack(CallbackRequestPayment result)
        {
            ...
            {
                _panel.UpdatePayment(factorID, DateTimeGenerator.GetShamsiDate(), DateTimeGenerator.GetShamsiTime(), verifyRes.Decription, "سداد ملی", verifyRes.SystemTraceNo, verifyRes.RetrivalRefNo);
